...
type testData struct {
	...
	userWithMod *models.User
	userWithout *models.User
...
func loadTestData(c context.Context, t *testing.T) (container testcontainers.Container, connector database.Connector, data testData) {
	...
	// users
	data.userWithMod = &models.User{Email: "with_mod@test.com", Tag: "user_with_mod", Password: "x", Roles: []*models.Role{&roles[1]}}
	if err = connector.GormDB().Create(data.userWithMod).Error; err != nil {
		return
	}

	data.userWithout = &models.User{Email: "without_mod@test.com", Tag: "user_without_mod", Password: "x"}
	if err = connector.GormDB().Create(data.userWithout).Error; err != nil {
		return
	}

	data.userAdmin = &models.User{Email: "admin@test.com", Tag: "admin_user", Password: "x", Roles: []*models.Role{&roles[0], &roles[1]}}
...
func TestIntegration_Success(t *testing.T) {
	...
	// revoke MODERATOR from user who has it
	req := httptest.NewRequest(Method, "/"+data.userWithMod.Tag+"/roles/"+string(models.RoleModerator), nil)
	...
	if err := connector.GormDB().Model(&models.UserRole{}).
		Where("user_id = ? AND role_id = ?", data.userWithMod.ID, data.roles[models.RoleModerator].ID).
		Count(&cnt).Error; err != nil {
		t.Fatal(err)
	}
	if cnt != 0 {
		t.Fatalf("expected user role to be deleted")
	}
}
...
func TestIntegration_ErrConflict(t *testing.T) {
	...
	// user does not have MODERATOR
	req := httptest.NewRequest(Method, "/"+data.userWithout.Tag+"/roles/"+string(models.RoleModerator), nil)
	res, err := app.Test(req)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegration_ErrForbidden(t *testing.T) {
	c := context.Background()
	container, connector, data := loadTestData(c, t)
	t.Cleanup(func() { test_utils.CleanUpTestData(c, t, container, connector) })
	...
	// target is ADMIN -> forbidden
	req := httptest.NewRequest(Method, "/"+data.userAdmin.Tag+"/roles/"+string(models.RoleModerator), nil)
	res, err := app.Test(req)
	if err != nil {
		t.Fatal(err)
	}
